...
const Footer = () => {
  return (
    <div className='w-full flex justify-center items-center bg-primary-text-contrast-two flex-col'>
      <div className='w-full max-w-screen-xl flex justify-between flex-col md:flex-row gap-8 md:gap-16 py-9 md:py-14 px-3 md:px-8 2xl:px-0'>
        <div className='w-full max-w-44 flex flex-col gap-3'>
          <h1 className='text-xl text-bg-base font-semibold'>
          ...
          {data.footer.topLinks.map((item) => (
            <a
              key={item.name}
              href={item.href}
              className=' text-sm font-medium text-fg-border hover:text-bg-base'
            >
              {item.name}
            </a>
          ))}
